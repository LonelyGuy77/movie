...
<body>
	...
	<div class="help_section_bottom" align="center">
		<div class="help_select">
			<a href="List.do">1:1 문의</a><a href="ListBest.do">자주 묻는 질문</a>
		</div>
		<form action="ListSearch.do" method="get">
			...
			<div>
				<c:if test="${paging.leftOn==true}">
					<a class="left_on"
						href="List.do?page=${paging.firstPage-paging.maxPage}">◀</a>
				</c:if>
				...
				<c:forEach var="num" begin="${paging.firstPage}"
					...
					<c:if test="${page!=num}">
						<a class="page_num" page_num="${num}" href="List.do?page=${num}">${num}</a>
					</c:if>
				...
				<input type="button" style="WIDTH: 60pt; HEIGHT: 20pt" value="글쓰기"
					...
					onclick="window.location='List.do'"></input>
			</div>
	</div>
